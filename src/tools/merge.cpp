...
void Merge::cli_prepare(CLI::App * app) {
	...
	CLI::Option * out_option = subapp->add_option("-o, --outfile", output_filename, "Kero file where all the input will be merged");
	out_option->required();
}

void Merge::merge(const vector<string> inputs, string output) {
	vector<Kero_file *> files;
	files.reserve(inputs.size());
	for(auto & input : inputs) {
    files.push_back(new Kero_file(input, "r"));
	}

	this->merge(files, output);

    for (Kero_file* file : files)
    {
        delete file;
    }
	files.clear();
...
void Merge::merge(const vector<Kero_file *> & files, string output) {
	// Useful variables
	const long buffer_size = 1048576; // 1 MB
	uint8_t buffer[1048576];
	uint8_t global_encoding[4];

	Kero_file * tmpFile = files[0];
	tmpFile->open("r");
	...
	int cnt = 0;
	for (Kero_file * infile : files) {
		infile->open("r");
		// Encoding verification
		for (uint i=0 ; i<4 ; i++) {
			if (infile->encoding[i] != global_encoding[i]) {
				cerr << "Wrong encoding for file " << infile->filename << endl;
				cerr << "Its nucleotide encoding is different from previous kero files." << endl;
				cerr << "Please first use 'kero-tools translate' to have the same encoding" << endl;
				exit(1);
			}
		}
		...
		while(infile->tellp() != infile->end_position) {
			...
			switch (section_type) {
                ...
                // No need to register all the types of sections in the index section
				// Register the section in the index of the output file
//				outfile.register_position(section_type);

                // Register the minimizer section
                if (section_type == 'M') {
                    // Read the minimizer
                    begin_byte = infile->tellp();
                    Section_Minimizer sm(infile);
                    outfile.register_minimizer_section(kero::mask_mini(sm.minimizer, sm.m));
                	sm.close();
                    infile->jump_to(begin_byte);
                }
		...
		infile->close();
		// cout << "File " << infile->filename << " merged (" << ++cnt << "/" << files.size() << ")" << endl;
		delete infile;
	}
