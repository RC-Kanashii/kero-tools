...
void Shuffle::cli_prepare(CLI::App * app) {
	...
	CLI::Option * out_option = subapp->add_option("-o, --outfile", output_filename, "Shuffled output KERO file");
	out_option->required();
}
...
//                    random_shuffle(everything.begin(),everything.end());
                    ...
                    // Use Mersenne Twister as random engine
                    std::default_random_engine rng(seed);
                    // Shuffle the vector
                    std::shuffle(everything.begin(), everything.end(), rng);
