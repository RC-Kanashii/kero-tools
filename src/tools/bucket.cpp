...
void Bucket::exec() {
	...
	KeroSeqStream stream(this->input_filename);
	...
	uint max_seq = 1;uint8_t * subseq = new uint8_t[(max_seq + 3) / 4];
	...
	while(true) {
		...
		if (nb_kmers == 0)
			...
			if (new_seq_size > max_seq) {
				...
				subseq = new uint8_t[(max_seq + 3) / 4];
			}
        ...
        for (skmer sk : skmers) {
			...
			if (buckets[mutex_idx].find(sk.minimizer) == buckets[mutex_idx].end()) {
				...
				// Create the file
				Kero_file * outfile = new Kero_file(output_filename + "_" + to_string(sk.minimizer) + ".kero", "w");
	...
	for (uint mutex_idx=0 ; mutex_idx<nb_mutex ; mutex_idx++)
		for (auto& it: buckets[mutex_idx]) {
			...
			Kero_file * outfile = sm->file;
			sm->close();
			delete sm;

			files.push_back(outfile);
			outfile->close(false);
			// Don't open all the files at the same time
			// outfile->open("r");
		}
