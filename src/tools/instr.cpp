...
void Instr::cli_prepare(CLI::App * app) {
	this->subapp = app->add_subcommand("instr", "Convert a text kmer file or a text sequence file into a kero file. Kmers or sequences must be 1 per line. If data size is more than 0, then the delimiters are used to split each line.");
	...
	CLI::Option * output_option = subapp->add_option("-o, --outfile", output_filename, "The kero output file name.");
