...
void Index::cli_prepare(CLI::App * app) {
  ...
  CLI::Option * out_option = subapp->add_option("-o, --outfile", output_filename, "Indexed kero to write (must be different from the input)");
  out_option->required();
}


void Index::exec() {
  Kero_file infile(this->input_filename, "r");
