...
// Use type aliases to simplify code and improve readability
// KmerDataPtr is a unique pointer to a uint8_t array, representing kmer data
using KmerDataPtr = std::unique_ptr<uint8_t[]>;
// KmerMatrix is a 2D vector where each element is a unique pointer to kmer data
using KmerMatrix = std::vector<std::vector<KmerDataPtr>>;

class Compact: public KeroTool {
private:
...
public:
	 ...
	 **/
	void write_paths(const std::vector<std::vector<const uint8_t*>> & paths, Section_Minimizer & sm, uint data_size);
	 ...
	 **/
	std::vector<std::vector<const uint8_t*>> pairs_to_paths(const std::vector<std::pair<const uint8_t*, const uint8_t*>> & to_compact);
	...
	KmerMatrix prepare_kmer_matrix(Section_Minimizer & sm);
	 ...
	 * position.
	 **/
	void sort_matrix(KmerMatrix& kmer_matrix);
	 ...
	 **/
	std::vector<std::pair<uint64_t, uint64_t>> pair_kmers(const std::vector<KmerDataPtr> & column1, const std::vector<KmerDataPtr> & column2) const;
	 ...
	 **/
	std::vector<std::vector<const uint8_t*>> polish_sort(const KmerMatrix & matrix, const std::vector<std::vector<std::pair<uint64_t, uint64_t>>> & pairs) const;
	 ...
	 **/
	std::vector<std::vector<const uint8_t*>> sorted_assembly(KmerMatrix & kmers);
	 ...
	 * @return Each pair of linked kmers. If a kmer is not linked one of the two values is a nullpointer.
	 **/
	std::vector<std::pair<const uint8_t*, const uint8_t*>> greedy_assembly(KmerMatrix & kmers);
	...
	/** Read a Section_Raw and write a bucketized and compacted file of the kmers.
	  ...
	  */
	void exec();
	void compact_section(Section_Minimizer & ism, Kero_file & outfile);
