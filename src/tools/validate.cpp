...
void Validate::cli_prepare(CLI::App * app) {
	...
	subapp->add_option("-o, --outfile", output_filename, "The file to write the results to")
		->required();
...
void Validate::exec() {
	...
	if (!file.is_open()) {
		cerr << "Cannot open the output file " << output_filename << endl;
		return;
	}

	// Get the default cout stream buffer
	...
	try {
		...
		if (verbose) {
			cout << "=== Header ===" << endl;
			// Important values
			cout << "-> KERO file version " << (uint)infile.major_version << "." << (uint)infile.minor_version << endl;
		...
		while(infile.tellp() < infile.end_position) {
            ...
            // Hashtable section
                ...
                if (not this->is_valid_h_section(infile))
                    exit(1);
            }
			// Unknown section
			else {
		...
		char kero[3];
		infile.read((uint8_t *)kero, 3);
		if (kero[0] != 'K' or kero[1] != 'E' or kero[2] !='R') {
			cout << "No KERO signature found at the end of the file. The file must be corrupted." << endl;
		}
...
bool Validate::is_valid_r_section(Kero_file & infile) {
...
bool Validate::is_valid_M_section(Kero_file & infile) {
    ...
    if (verbose) {
    	...
    	cout << "-> Offset of n column: " << sm.n_col_offset << endl;
    	cout << "-> Offset of m_idx column: " << sm.m_idx_col_offset << endl;
    	cout << "-> Offset of seq column: " << sm.seq_col_offset << endl;
    	if (sm.data_size > 0) {
    		cout << "-> Offset of data column: " << sm.data_col_offset << endl;
    	}
        cout << endl;
    }
...
bool Validate::is_valid_i_section(Kero_file & infile) {
...
bool Validate::is_valid_h_section(Kero_file & infile) {
    Section_Hashtable sh(&infile);

    if (verbose) {
        cout << "Start Byte " << sh.beginning << endl;
    	cout << "-> Size of the mpht: " << sh.nb_mphf << endl;
        cout << "-> Number of minimizers: " << sh.mpht.size() << endl;

		uint64_t m = infile.global_vars["m"];
    	for (auto minimizer : sh.minimizers) {
    		uint64_t pos = sh.mpht.find(minimizer);
    		cout << "* " << strif.translate(minimizer, m) << " -> " << pos << endl;
    	}
    }
